...
function pointCollidesWithBody(px, py, body, startIndex = 0) {
  for (let i = startIndex; i < body.length; i++) {
    if (Math.hypot(px - body[i].x, py - body[i].y) < PLAYER_RADIUS) {
      return true;
    }
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('input', ({ angle, boost }) => {
    ...
    if (Math.abs(diff) > maxDiff) {
      desired = normalizeAngle(
        player.angle + Math.sign(diff) * maxDiff
      );
    }
...
setInterval(() => {
  ...
  gameState.players.forEach((player) => {
    ...
    player.x = Math.max(
      20,
      Math.min(WORLD_SIZE - 20, newX)
    );

    player.y = Math.max(
      20,
      Math.min(WORLD_SIZE - 20, newY)
    );
  ...
  if (occupiedFoods.size) {
    gameState.foods =
      gameState.foods.filter(
        f => !occupiedFoods.has(f.id)
      );
  }
    ...
    for (const key of getNeighborKeys(player.x, player.y)) {
      ...
      for (const other of neighbors) {
        ...
        if (
          other.id === player.id ||
          !other.alive ||
          checkedPlayers.has(other.id)
        ) continue;

        checkedPlayers.add(other.id);

        if (
          pointCollidesWithBody(
            player.x,
            player.y,
            other.body,
            BODY_SKIP
  ...
  deadPlayers.forEach(({ victim, killer }) => {
...
httpServer.listen(3000, () => {
  console.log('Arena Server running on :3000');
});
